...
def py_weighted_median(data, weights):
    ...
    if any(weights > midpoint):
        ...
        try:
            idx_upper = np.intersect1d(np.where(cs_weights > midpoint)[0],
                                       np.where(cs_weights != 0)[0], True)[0] + 1
        except IndexError:
            idx_upper = len(data)
            # skip zero-weights
            try:
                ...
                pass
        try:
            idx_lower = np.intersect1d(np.where(cs_weights[-1] - cs_weights > midpoint)[0],
                                       np.where(cs_weights != 0)[0], True)[-1] + 1
        except IndexError:
            idx_lower = 0
            # skip zero-weights
            try:
...
def py_mwm_1d(values, weights, size):
    ...
    for i in range(len(values)):
        ...
        # window is sliding into the value array
            ...
            medians.append(py_weighted_median(values[: i + size // 2 + 1],
                                              weights[: i + size // 2 + 1]))

        # window is sliding over the end of the value array
        elif i + size // 2 >= len(values):
            medians.append(py_weighted_median(values[i - size // 2:], weights[i - size // 2:]))

        # the normal case: window is inside the value array
        else:
            medians.append(py_weighted_median(values[i - size // 2:i + size // 2 + 1],
...
def py_mwm_nd(values, weights, size):
    ...
    for index, value in np.ndenumerate(values):
        ...
        medians[index] = py_weighted_median(values[window].flatten(), weights[window].flatten())
    return medians


class TestMWM(unittest.TestCase):
    def test_the_test(self):
        mwm = py_mwm_1d(values=[1, 2, 3, 4, 5, 6, 7, 8],
                        weights=[1, 2, 3, 4, 5, 6, 7, 8], size=3)
        assert mwm == [2, 2.5, 3, 4, 5, 6, 7, 8]

        assert (py_weighted_median([1, 2, 3, 4, 5, 6, 7, 8],
                                   [1, 0, 0, 0, 0, 0, 0, 1]) == 4.5)
    ...
    def test_the_nd_test(self):
        ...
        np.testing.assert_array_equal([[2.0, 2.5, 3.0],
                                       [2.5, 2.5, 2.5],
                                       [3.0, 2.5, 2.0]],
                                      mwm)

    def test_wm(self):
        assert (weighted_median([1, 2, 3, 4, 5, 6, 7, 8],
                                [1, 0, 0, 0, 0, 0, 0, 1]) == 4.5)

        assert (weighted_median([1, 2, 3, 4, 5, 6, 7, 8],
                                [1, 0, 0, 0, 0, 0, 0, 1], method='lower') == 1)

        assert (weighted_median([1, 2, 3, 4, 5, 6, 7, 8],
                                [1, 0, 0, 0, 0, 0, 0, 1], method='higher') == 8)

        values = np.asarray([1, 2, 3, 4, 5, 6, 7, 8], dtype=np.float64)
        weights = np.asarray([1, 2, 3, 4, 5, 6, 7, 8], dtype=np.float64)
        np.testing.assert_array_equal(py_weighted_median(values, weights),
                                      weighted_median(values, weights))

        values = np.asarray([[1, 2, 3],
                             [2, 3, 1],
                             [3, 1, 2]], dtype=np.float64)
        np.testing.assert_array_equal(py_weighted_median(values, values),
                                      weighted_median(values, values))
    ...
    def test_1d_mwm_int(self):
        ...
        np.testing.assert_array_almost_equal(py_mwm_1d(values, weights, 5),
    ...
    def test_1d_mwm(self):
        ...
        np.testing.assert_array_equal(py_mwm_1d(values, weights, 5),
                                      moving_weighted_median(values, weights, 5))
...
# These two are just for measuring performance:
    def test_1d_mwm_big(self):
        ...
        print('1D moving weighted median with {} elements took {}s / {}s'
              .format(N, t_py, t_cython))
    ...
    def test_zero_weights(self):
        ...
        np.testing.assert_array_equal([0, 0, 0],
                                      moving_weighted_median(values, weights, 1))
    ...
    def test_2d_mwm_int(self):
        ...
        np.testing.assert_array_equal(py_mwm_nd(values, weights, (3, 3)),
                                      moving_weighted_median(values, weights, (3, 3)))
