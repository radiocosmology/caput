...
Implements core pipeline infrastructure.
"""

__all__ = ["Manager", "PipelineRuntimeError", "PipelineStopIteration", "TaskBase"]
...
from .. import config, misc, mpiutil, profile, tools
...
class Manager(config.Reader):
    ...
    def from_yaml_str(cls, yaml_doc, lint=False, psutil_profiling=False):
        ...
        self: Pipeline object
        """
        yaml_params = yaml.load(yaml_doc, Loader=config.SafeLineLoader)
        try:
    ...
    def _next_task(self):
        ...
        for ii in range(len(self.tasks)):
            ...
            if task.__pipeline_is_available:
                available.append(jj)
        ...
        new_index = sorted(
            available,
            key=lambda i: (self.tasks[i].__priority, self.tasks[i].base_priority),
            reverse=True,
        )[0]
...
class TaskBase(config.Reader):
    ...
    @property
    def __pipeline_is_available(self):
