r"""Scalable, MPI-parallel data pipeline infrastructure.
...
Pipelines are most easily executed using the `caput-pipeline` command-line interface.

Basic Tasks
~~~~~~~~~~~
A pipeline task is a subclass of :py:class:`.Task` intended to perform some small,
modular piece of analysis. The developer of the task must specify what input
...
instances of :py:class:`~caput.config.Property`. For instance a task definition
might begin with
...
:py:meth:`~.Task.setup`, :py:meth:`~.Task.next` or
:meth:`~.Task.finish` methods (:py:meth:`~.Task.__init__` may also be
implemented).  These are executed in order, with :py:meth:`~.Task.next`
possibly being executed many times.  Iteration of :py:meth:`~.Task.next` is halted by
raising a :py:exc:`~.exceptions.PipelineStopIteration`.  Here is a example of a somewhat
...
and provided to subsequent tasks. The methods :py:meth:`~.Task.setup` and
:py:meth:`~.Task.next` may accept (positional only) arguments which will be
received as the outputs of :py:meth:`~.Task.next` of early tasks in a pipeline
chain. The following is an example of a pair of tasks that are designed to operate
in this manner.
...
Note that `CookEggs.next` never raises a :py:exc:`~.exceptions.PipelineStopIteration`.
This is because there is no way for the task to internally know how long to
iterate.  :py:meth:`~.Task.next` will continue to be called as long as there are inputs
for :py:meth:`~.Task.next` and will stop iterating when there are none.
...
``out``
    ...
    :py:meth:`~.Task.setup`, :py:meth:`~.Task.next` or :py:meth:`~.Task.finish`.
``requires``
    A 'pipeline product key' or list of keys representing values to be passed
    as arguments to :py:meth:`~.Task.setup`.
``in_``
    A 'pipeline product key' or list of keys representing values to be passed
    as arguments to :py:meth:`~.Task.next`.
...
>>> from caput.pipeline import local_tasks, Manager
...
1. One of the methods :py:meth:`~.Task.setup`, :py:meth:`~.Task.next` or
   :py:meth:`~.Task.finish`, as appropriate, will be executed from each task, in order.
2. If the task method is missing its input, as specified by the ``requires`` or ``in_``
   keys, restart at the beginning of the ``tasks`` list.
3. If the input to :py:meth:`~.Task.next` is missing and the task is at the beginning of the
   list there will be no opportunity to generate this input. Stop iterating
   :py:meth:`~.Task.next` and proceed to :py:meth:`~.Task.finish`.
4. Once a task has executed :py:meth:`~.Task.finish`, remove it from the list.
...
The following would error, because the pipeline config is checked for errors, like an
``in_`` parameter without a corresponding ``out``:

>>> m = Manager.from_yaml_str(new_spam_config) # doctest: +SKIP
>>> m.run() # doctest: +SKIP
...
generates its input, 'non_existent_data_product'. Once everything before `DoNothing` has
been executed the pipeline notices that there is no opportunity for 'non_existent_data_product'
to be generated and forces `DoNothing` to proceed to :py:meth:`~.Task.finish`. This also
unblocks :py:class:`PrintEggs` allowing it to proceed normally.
...
configuration attributes directly (or call :py:meth:`~.Task.read_config` on an
appropriate dictionary); and add it to the pipeline using the
:py:meth:`~.Manager.add_task` to add the instance and specify the queues it connects to.

To inject products into the pipeline, use :py:class:`~.extensions.Input` and supply it an
iterator as an argument. Each item will be fed into the pipeline one by one. To take
outputs from the pipeline, simply use the :py:class:`~.extensions.Output` task. By default this
...
Advanced Tasks
~~~~~~~~~~~~~~
Several subclasses of :py:class:`.Task` provide advanced functionality for tasks that
...
optionally writing outputs to disk automatically.
"""
