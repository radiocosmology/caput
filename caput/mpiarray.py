...
class MPIArray(np.ndarray):
    ...
    def _view_from_data_and_params(
        ...
        -------
        dist_arr
            MPIArray view of the numpy array
        """
        # Set shape and offset
        local_shape = array.shape
        ...
        dist_arr._axis = axis
        dist_arr._local_shape = tuple(local_shape)
        dist_arr._local_offset = tuple(local_offset)
        dist_arr._comm = comm
    ...
    def redistribute(self, axis: int) -> MPIArray:
        ...
        # Must be able to allocate the target array and 2 buffers. Allocate
        ...
        if csize == 1:
            # There is only one MPI process so all data is already local
        ...
        # Split the source array into properly sized blocks for sending
    ...
    def copy(self, *args, **kwargs) -> MPIArray:
        ...
        return MPIArray.wrap(
            self.local_array.copy(*args, **kwargs), axis=self.axis, comm=self.comm
        )

    def enumerate(self, axis: int):
    ...
    def from_zarr(cls, f, dataset, comm=None, axis=0, sel=None) -> MPIArray:
        ...
        array
        """
        return cls.from_file(f, dataset, comm, axis, sel, file_format=fileformats.Zarr)
