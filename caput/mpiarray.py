    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        ...
        for input_ in inputs:
            if isinstance(input_, MPIArray):
                if axis is None:
                    ...
                    assert axis == input_.axis, "The distributed axis for all MPIArrays in an exp should be the same"
        ...
        for mpi_array in input_mpi:
            if hasattr(kwargs, 'axis') and (kwargs['axis'] == mpi_array.axis):
                raise ValueError(f'operations along the distributed axis (in this case, {mpi_array.axis}) are not allowed.')
        ...
        results = tuple(MPIArray.wrap(result, axis=axis) if output is None else output for result, output in zip(results, outputs))
